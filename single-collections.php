...
?>
    ...
    <?php endif; ?>
    <?php
    if ($post_id = get_field('blog_to_display')): // Get the selected post ID
        // Fetch post details
        $post = get_post($post_id);
        if ($post): // Ensure the post exists
            $featured_image = get_the_post_thumbnail_url($post_id, 'full'); // Get featured image URL
            $excerpt = get_the_excerpt($post_id); // Get the excerpt
            $categories = get_the_category($post_id); // Get categories
            $permalink = get_permalink($post_id); // Get the post URL
    ?>
