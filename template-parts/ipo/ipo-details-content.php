...
// Helper function to get IPO data from ACF repeater field
function get_ipo_acf_data($ipo_id, $field_name = null) {
    $ipo_list = get_field('ipo_list', 'option');
    if ($ipo_list) {
        foreach ($ipo_list as $row) {
            if (($row['ipo_id'] ?? '') === $ipo_id) {
                return $field_name ? ($row[$field_name] ?? false) : $row;
            }
        }
    }
    return false;
...
if ($ipo) {
    ...
    if (!empty($ipo->api_spv_id)) {
        $api_spv_id = $ipo->api_spv_id;
    }
    if (!empty($ipo->api_spv_synopsis)) {
        $api_spv_synopsis = $ipo->api_spv_synopsis;
    }
}
		...
		<div class="ipo_main_wrapper">
			<div class="ipo_content">
				...
				<div class="ipo_tabs">
					...
					<?php endif; ?>
					<?php if ((!empty($ipo->description) && trim($ipo->description) !== 'description') || (!empty($api_spv_synopsis) && trim($api_spv_synopsis) !== '')): ?>
				...
				<div class="ipo_content_box" id="ipo-overview">
					...
					<div class="ipo_key_information">
						<div class="ipo_ki_metrics">
							...
							<div class="ip_ki_metric">
								<?php 
									// Check for ACF override first, then use database value
									$acf_valuation = get_ipo_acf_data($ipo_id, 'ipo_valuation');
									$final_valuation = $acf_valuation ?: $api_valuation;
									
									if (strpos($final_valuation, '$') === 0) {
										echo '<h4>' . esc_html($final_valuation) . '</h4>';
									} else {
										echo '<h4>$' . esc_html($final_valuation) . '</h4>';
									}
								?>
						...
						<div class="ipo_ki_meta_container">
							<div class="ipo_ki_meta_box">
								...
								<?php endif; ?>
								<?php if ($api_all_in_price_per_share !== 'N/A'): ?>
									<div class="ipo_ki_meta">
										<span>
								...
								<?php endif; ?>
								<?php 
									// Get ACF override dates
									$acf_close_date = get_ipo_acf_data($ipo_id, 'ipo_close_date');
									$final_close_date = $acf_close_date ?: $api_funding_deadline;
								?>
								<?php if ($final_close_date && $final_close_date !== 'N/A'): ?>
								<div class="ipo_ki_meta">
									<span>Close Date</span>
										<strong><?php echo format_date_with_ordinal($final_close_date); ?></strong>
								</div>
								<?php endif; ?>
							</div>
							<div class="ipo_ki_meta_box">
				...
				<div class="ipo_content_box" id="ipo-about">
					...
					<div class="ipo_about_content">
						<?php
						// Use SPV synopsis if available, otherwise fall back to description
						if (!empty($api_spv_synopsis) && trim($api_spv_synopsis) !== '') {
							$content = esc_html($api_spv_synopsis);
						} else {
							$content = esc_html($ipo->description);
						}
				...
				<div class="ipo_content_box">
					...
					?>
						...
						<div class="ipo_faq_item ipo_faq_item_hidden">
							...
							<div class="ipo_faq_answer">
								<p>The offering is priced at $<?php echo $api_price_per_share !== 'N/A' ? $api_price_per_share : 'TBD'; ?>/share, implying a valuation of approximately $<?php 
									$acf_valuation = get_ipo_acf_data($ipo_id, 'ipo_valuation');
									$final_valuation = !empty($acf_valuation) ? $acf_valuation : $api_valuation;
									echo $final_valuation !== 'N/A' ? $final_valuation : 'TBD'; 
								?>. This includes a one-time management fee and expense reserve. There are no ongoing fees or carry.</p>
							</div>
						</div>
						<?php endif; ?>
						<?php if ($has_investors_data): ?>
						<div class="ipo_faq_item ipo_faq_item_hidden">
							...
							<div class="ipo_faq_answer">
								<?php 
									// Get ACF override dates for FAQ
									$acf_open_date_faq = get_ipo_acf_data($ipo_id, 'ipo_open_date');
									$acf_close_date_faq = get_ipo_acf_data($ipo_id, 'ipo_close_date');
									$faq_open_date = $acf_open_date_faq ?: ($ipo->year_est ?? '');
									$faq_close_date = $acf_close_date_faq ?: $api_funding_deadline;
								?>
								<p>The offering for <?php echo esc_html($ipo->name ?? 'this company'); ?> opens on <?php echo $faq_open_date ? format_date_with_ordinal($faq_open_date) : 'TBD'; ?> and closes on <?php echo $faq_close_date !== 'N/A' ? format_date_with_ordinal($faq_close_date) : 'TBD'; ?>. Once closed, the opportunity will no longer be available for subscription.</p>
							</div>
						</div>
						<?php endif; ?>
						<div class="ipo_faq_item ipo_faq_item_hidden">
			...
			<div class="ipo_sidebar">
				<div class="ipo_sidebar_box quick_actions">
					...
					<div class="ipo_quick_actions <?php echo ($has_investors_data && !empty($ipo->api_deal_memo_url)) ? 'with_deal_memo' : ''; ?>">
						<?php if ($has_investors_data): ?>
							...
							?>
							<a href="<?php echo esc_url($invest_url); ?>" class="ipo_primary_button" id="invest-button-spv">
								Invest
							</a>
						<?php else: ?>
							<?php $typoform_link = get_ipo_acf_data($ipo_id, 'ipo_typoform_link'); ?>
							<?php 
								$ipo_row = get_ipo_acf_data($ipo_id);
								$typoform_button_text = $ipo_row ? ($ipo_row['ipo_typoform_button_text'] ?? $ipo_row['ipo_primary_button_text'] ?? $ipo_row['typoform_button_text'] ?? '') : '';
							?>
							<a href="<?php if ($typoform_link) { echo esc_url($typoform_link); } else { echo 'https://vestedfinance.typeform.com/to/NBg1K5gi'; } ?>" class="ipo_primary_button">
								<?php if ($typoform_button_text) { echo esc_html($typoform_button_text); } else { echo 'Express Interest'; } ?>
							</a>
						<?php endif; ?>
				...
				<?php error_log('documents_data: ' . print_r($documents_data, true)); ?>
				...
				<?php else: ?>
                    <?php error_log('has_investors_data: ' . print_r($has_investors_data, true)); ?>
					<?php if ($funding_rounds_data && !empty($funding_rounds_data['items']) && empty($has_investors_data)): ?>
                        <div class="ipo_sidebar_box">
