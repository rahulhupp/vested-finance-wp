...
/**
 * Time-based block for /wp-json/wp/v2/posts.
 *
 * - Production window: 19:00–20:00 IST
 */
add_filter(
	...
	function ( $result ) {
		...
		// Only block posts endpoint (any query string).
		if ( strpos( $request_uri, '/wp-json/wp/v2/posts' ) === false ) {
			return $result;
		}

		// Current time in IST (fixes cases where site TZ is left at UTC).
		$now_ts = time(); // server time (UTC).
		...
		// Windows in minutes since midnight (24h).
		$windows = array(
			array( 'start' => 19 * 60 + 0,  'end' => 20 * 60 + 0 ),
			array( 'start' => 16 * 60 + 15, 'end' => 16 * 60 + 30 ),
		);

		foreach ( $windows as $window ) {
			if ( $now_minutes >= $window['start'] && $now_minutes < $window['end'] ) {
				return new WP_Error(
					'protected',
